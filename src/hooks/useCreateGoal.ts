...
export const useCreateGoal = (navigation: any) => {
    // Refs
    ...
    const loadNutrients = async () => {
        try {
            ...
            const data = await getNutrients();
            setNutrients(data);
        } catch (error: any) {

            Alert.alert('Lỗi', 'Không thể tải danh sách chất dinh dưỡng. Vui lòng thử lại.');
        } finally {
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            await healthGoalService.createCustom(requestData);
            
            Alert.alert(
                'Thành công',
                `Đã tạo mục tiêu "${goalName}"`,
                [{ text: 'OK', onPress: () => navigation.goBack() }]
            );

        } catch (error: any) {
            Alert.alert('Lỗi', error?.message || 'Không thể tạo mục tiêu. Vui lòng thử lại.');
        } finally {
            setIsSaving(false);
        }
    };
